	...
	public static void processText(String text) {
		...
		for (int i = 1; i < bits.length; i++) { // This just won't run if there aren't any arguments.
			if (currentlyQuoted) {
				...
				lstArgs.set(lstArgs.size() - 1, lstArgs.get(lstArgs.size() - 1) + " " + bits[i]); // Set the last element to its previous text plus space plus
																									// this bit
			} else {
		...
		/*
		 * System.out.println("Command: " + command); for(String f: lstFlags) { System.out.println("Flag: " + f); } for(String a: lstArgs) {
